...
class MainActivity : AppCompatActivity() {
    ...
    lifecycleScope.launch {
      ...
      repeatOnLifecycle(Lifecycle.State.STARTED) {
        viewModel.moviesListState.collect { state ->
          when (state) {
            is UIState.Error -> Toast.makeText(this@MainActivity, "Error", Toast.LENGTH_SHORT).show()
            UIState.Loading -> Toast.makeText(this@MainActivity, "Loading", Toast.LENGTH_SHORT).show()
            is UIState.Success -> {
  ...
  private fun loadMore(firstTime: Boolean, rv: RecyclerView, items: List<CategoryRow>) {
    isLoading = true
